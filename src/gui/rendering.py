...
class Renderer(metaclass=ABCMeta):  # MixIn
    ...
    #    self.update_viewbox_size()

    #def update_viewbox_size(self):
    #    # Get the current window size
    #    axis_width = self.plot_area.main_window.axis_width

    #    # Set the new size for the GridItem
    #    self.gr.setGeometry(QtCore.QRectF(0, 0, axis_width, 100))
...
class Waveform(Renderer):
    ...
    def generateBlankPlotItems(self):
        ...
        self.ax = pg.AxisItem('left', showValues=True)  # ticks are disabled, because each view has separately created GridItem()
        self.configNewAxis()
        #from gui import PALMS
        #if len(PALMS.get().viewer.getRRDisplayPanel().panel.views) > 1:
        #    previous_viewer = PALMS.get().viewer.getRRDisplayPanel().panel.views[0]
        #    self.vb = PALMS.get().viewer.getRRDisplayPanel().plot_area.vbs[previous_viewer]
        #else:
