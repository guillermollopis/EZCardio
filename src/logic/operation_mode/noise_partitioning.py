...
class NoisePartition(pg.LinearRegionItem):
    """
    ...
    def __init__(self, name: str, *, start: float = None, end: float = None, click: bool = False):
        ...
        #if (click):
        left_bound, right_bound = NoisePartitions.calculate_boundaries(start, end)
        ...
        NoisePartitions.add(self)
        
        NoisePartitions.update_all_bounds(self)
    ...
    def from_click(cls, name: str, *, click_x: float, click: bool = False):
        ...
        left_bound, right_bound = NoisePartitions.calculate_boundaries(click_x, click_x)
        left_bound = left_bound if left_bound is not None else track.minX
        right_bound = right_bound if right_bound is not None else track.maxX
    ...
    def from_start_end(cls, name: str, *, start: int, end: int, click: bool = True):
        ...
        start = PALMS.get().from_sample_to_time(start)
        end = PALMS.get().from_sample_to_time(end)
    ...
    def region_moved(self):
        ...
        NoisePartitions.remove_zero_partitions()
            
        NoisePartitions.update_all_bounds()
        ...
        from logic.operation_mode.rr_noise_partitioning import RRNoisePartitions
        rr_partition = RRNoisePartitions.find_partition_by_start(previous_start)[0]
        rr_partition.start = self.start
        rr_partition.end = self.end
        rr_partition.mid = rr_partition.start + (rr_partition.end - rr_partition.start) / 2
        rr_partition.setRegion([rr_partition.start, rr_partition.end])
    ...
    def region_deleted(self):
        ...
        if self is not None:
            ...
            NoisePartitions.delete(self)
            NoisePartitions.update_all_bounds()
            qInfo('Region {} [{:0.2f}; {:0.2f}] deleted'.format(self.name, self.start, self.end))
            # Update rr interval as new intra-signals
            rr_partition = RRNoisePartitions.find_partition_by_start(previous_start)[0]
            rr_partition.label.getViewBox().removeItem(rr_partition.label)
            rr_partition.getViewBox().removeItem(rr_partition)
            RRNoisePartitions.delete(rr_partition)
            RRNoisePartitions.update_all_bounds()



class NoisePartitions(QObject):
    """
    static class to operate on all existing SinglePartition instances
    """
    partitions: List[NoisePartition] = []  # keep sorted list of all SinglePartition instances

    def __new__(*args):  # instead of creating, return list of SinglePartition
        return NoisePartitions.partitions

    @staticmethod
    def remove_zero_partitions():  # if start==end --> SinglePartition is flat
        for p in NoisePartitions():
            x1, x2 = p.getRegion()
            if x1 == x2:
                p.region_deleted()

    @staticmethod
    def update_all_bounds(avoid_this_p: NoisePartition = None):
        """
        ...
        for p in NoisePartitions():
            if not p == avoid_this_p:
                left_bound, right_bound = NoisePartitions.calculate_boundaries(p.start, p.end)
    ...
    def calculate_boundaries(this_left: float, this_right: float):
        ...
        """
        nearest_left = bisect.bisect_right(NoisePartitions.all_endpoints(), this_left) - 1
        nearest_right = bisect.bisect_left(NoisePartitions.all_startpoints(), this_right)
        try:
            left_boundary = NoisePartitions()[nearest_left].end if nearest_left >= 0 else None
        except:
            left_boundary = None

        try:
            right_boundary = NoisePartitions()[nearest_right].start if nearest_right >= 0 else None
        except:
            right_boundary = None
    ...
    @staticmethod
    def add(p: NoisePartition):
        idx = bisect.bisect_left(NoisePartitions.all_midpoints(), p.mid)
        NoisePartitions.partitions.insert(idx, p)
    ...
    def add_all(labels: List[str], start: np.ndarray, end: np.ndarray):
        ...
        try:
            ...
            for l, s, e in zip(labels, start, end):
                NoisePartition(l, start=s, end=e)
        except Exception as e:
            Dialog().warningMessage('Partitions cannot be loaded\n' + str(e))

    @staticmethod
    def delete(p: NoisePartition):
        NoisePartitions.partitions.remove(p)

    @staticmethod
    def delete_all():
        NoisePartitions.hide_all_partitions()
        NoisePartitions.partitions = []

    @staticmethod
    def find_partition_by_point(click_x: float):  # get partition under mouse click or None
        idx = np.where((click_x >= NoisePartitions.all_startpoints()) & (click_x <= NoisePartitions.all_endpoints()))[0]
        if len(idx) == 1:
            return NoisePartitions()[idx[0]]
        elif len(idx) == 0:
            return None
        else:
    ...
    def find_partition_by_start(previous_start):  # get partition under mouse click or None
        ...
        filtered_objects = list(filter(lambda obj: PALMS.get().from_time_to_closest_sample(obj.start) == previous_start, NoisePartitions.partitions))

        if (filtered_objects == []):
            filtered_objects = list(filter(lambda obj: PALMS.get().from_time_to_closest_sample(obj.start)+1 == previous_start, NoisePartitions.partitions))
    ...
    @staticmethod
    def all_startpoints():
        return np.array([p.start for p in NoisePartitions.partitions])
    
    @staticmethod
    def all_startpoints_by_name(filter_name):
        filtered_objects = list(filter(lambda obj: obj.name == filter_name, NoisePartitions.partitions))
    ...
    def find_partitions_by_two_names(filter_name1, filter_name2):
        filtered_objects = list(filter(lambda obj: obj.name == filter_name1 or obj.name == filter_name2, NoisePartitions.partitions))
        return np.array(filtered_objects)
    ...
    @staticmethod
    def all_endpoints():
        return np.array([p.end for p in NoisePartitions.partitions])
    ...
    @staticmethod
    def all_midpoints():
        return np.array([p.mid for p in NoisePartitions.partitions])

    @staticmethod
    def all_labels():
        return [p.name for p in NoisePartitions.partitions]

    #@staticmethod
    #def unique_labels():
    #    return list(unique_everseen(NoisePartitions.all_labels()))

    @staticmethod
    def to_csv(filename: str):
        d = {'label': NoisePartitions.all_labels(), 'start': NoisePartitions.all_startpoints(), 'end': NoisePartitions.all_endpoints()}
    ...
    @staticmethod
    def clear_annotations_in_this_partition(p: NoisePartition):
        """
