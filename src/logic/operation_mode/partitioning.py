...
class SinglePartition(pg.LinearRegionItem):
    ...
    def __init__(self, name: str, *, start: float = None, end: float = None):
        ...
        start = max([start, track.minX])
        end = min([end, track.maxX])
...
class Partitions(QObject):
    ...
    def unhide_all_partitions():
        ...
        for i in Partitions():
            Viewer.get().RRDisplayPanel.plot_area.main_vb.addItem(i)
            Viewer.get().RRDisplayPanel.plot_area.main_vb.addItem(i.label)

    @staticmethod
    def unhide_all_partitions_by_name(filter_name):
        filtered_objects = list(filter(lambda obj: obj.name == filter_name, Partitions.partitions))
        from gui.viewer import Viewer
    ...
    def hide_all_partitions():
        from gui.viewer import Viewer
        for i in Partitions():
            Viewer.get().RRDisplayPanel.plot_area.main_vb.removeItem(i)
            Viewer.get().RRDisplayPanel.plot_area.main_vb.removeItem(i.label)

    @staticmethod
    def hide_all_partitions_by_name(filter_name):
        filtered_objects = list(filter(lambda obj: obj.name == filter_name, Partitions.partitions))
        from gui.viewer import Viewer
        for i in filtered_objects:
            Viewer.get().RRDisplayPanel.plot_area.main_vb.removeItem(i)
    ...
    @staticmethod
    def delete_by_name(filter_name):
        filtered_objects = list(filter(lambda obj: obj.name == filter_name, Partitions.partitions))
        for object in filtered_objects:
            object.region_deleted()
    ...
    @staticmethod
    def find_partition_by_name(filter_name):  # get partition under mouse click or None
        filtered_objects = list(filter(lambda obj: obj.name == filter_name, Partitions.partitions))
        
        sorted_objects = sorted(filtered_objects, key=lambda obj: obj.start)

        return sorted_objects
    
    @staticmethod
    def change_partition_name(previous_name, new_name):  # get partition under mouse click or None
        filtered_objects = list(filter(lambda obj: obj.name == previous_name, Partitions.partitions))
        list(map(lambda obj: setattr(obj, 'name', new_name), filtered_objects))
    ...
    @staticmethod
    def all_names():
        return list(set([p.name for p in Partitions.partitions]))
