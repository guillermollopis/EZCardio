...
def stress_index(nni=None, rpeaks=None, binsize=7.8125, plot=True, show=True, figsize=None, legend=True):

	rr_values = nni
	from gui import results_panel
	current_results_panel = results_panel.ResultsPanel.get()
	rr_values_ms = current_results_panel.get_stationary_rr(nni)
	...
	stress_value = round(np.sqrt(si_index), 3)

	args = (stress_value,)

	return biosppy.utils.ReturnTuple(args, ["stress_index"])
