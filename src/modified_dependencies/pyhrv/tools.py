...
# Local imports
import modified_dependencies.pyhrv as pyhrv
...
def time_varying(nni=None, rpeaks=None, parameter='sdnn', window='n60', sliding_window=60, minimum_effective_data=10, interpolation=None, show=True, mode='normal'):
	...
	current_result_panel = results_panel.ResultsPanel.get()
	import copy
	nn_detrended = copy.deepcopy(nn)
	if parameter not in ['nni_mean', 'hr_mean', 'hr_min', 'hr_max', 'hr_std', 'stress_index', 'fft_peak', 'fft_abs', 'fft_log', 'fft_rel', 'fft_norm', 'fft_total', 'fft_ratio', 'ar_peak', 'ar_abs', 'ar_log', 'ar_rel', 'ar_norm', 'ar_total', 'ar_ratio', 'lomb_peak', 'lomb_abs', 'lomb_log', 'lomb_rel', 'lomb_norm', 'lomb_total', 'lomb_ratio']: 
		nn_detrended = current_result_panel.get_stationary_rr(nn)
	nonlinear_detrend = current_result_panel.settings["nonlinear_detrending"]
	if nonlinear_detrend is False and parameter in ['sd1', 'sd2', 'sd_ratio', 'apen', 'sampen', 'dfa_alpha1', 'dfa_alpha2']:
		nn_detrended = current_result_panel.get_stationary_rr(nn)
	...
	def _compute_parameter(array, func):
		try:
			...
			val = eval(func + '(nni=array, mode=\'dev\')[\'%s\']' % parameter)
				...
				try:
					...
					if 'plot' in str(ae):
						...
						try:
							...
							val = eval(func + '(nn=array)[\'%s\']' % parameter)
		return val
	
	def is_too_noisy(start, end, minimum_data): # start and end are in time

		from logic.operation_mode.noise_partitioning import NoisePartitions

		minimum_sample_size = minimum_data # in time
	...
	# Time window computation
		...
		for i, _t in enumerate(result):
			...
			if _t <= window_size:
				...
				parameter_values = np.append(parameter_values, _compute_parameter(nn_detrended[indices], parameter_func))
			# Complete Window
	...
	if window_mode == 'n':
		...
		start_interval = np.where(result < window_size/1000)[0]
		valid_interval = np.where((result >= window_size/1000) & (result <= result[-1] - window_size/1000))[0]
		end_interval = np.where(result > result[-1] - window_size/1000)[0][:-1]
	...
	if PALMS.get().ORIGINAL_DATETIME == None:
		peaks_values_local = result # time axis values
		if result[-1] <= 60:
			ax.set_xlabel('Time [s]')
		# Set x-axis format to MM:SS if the duration of the signal > 60s and <= 1h
		elif 60 < result[-1] <= 3600:
			ax.set_xlabel('Time [MM:SS]')
			formatter = mpl.ticker.FuncFormatter(lambda ms, x: str(dt.timedelta(seconds=ms))[2:])
			ax.xaxis.set_major_formatter(formatter)	
		# Set x-axis format to HH:MM:SS if the duration of the signal > 1h	    
		...
		current_result = np.insert(result, 0, 0)
		from dateutil import parser
		from dateutil.relativedelta import relativedelta
		dt = parser.parse(PALMS.get().ORIGINAL_DATETIME)
		peaks_values_local = [dt + relativedelta(seconds=seconds) for seconds in current_result[:-1]]
		ax.set_xlabel('Time [d  HH:MM:SS]')
	    

	# Plot start values (except the very first NNI)
	ax.plot(peaks_values_local, parameter_values, 'g')

	# Plot valid values
	ax.plot(peaks_values_local[valid_interval], parameter_values[valid_interval], 'g')

	# Plot final values
	ax.plot(peaks_values_local[end_interval], parameter_values[end_interval], 'g')
	...
	parameter_minimum = minimum_effective_data
	for i, _t in enumerate(result):
		if i == 0:
			continue
		if is_too_noisy(result[i-1], _t, parameter_minimum): # use start of window, end of window and minimum size without noise
			#ax.vlines(parameter_minimum, y_min, y_max, color='red')
			min_x_value = max(0, result[i-1] - (_t - result[i-1])/2)
			max_x_value = min(result[-1], result[i-1] + (_t - result[i-1])/2)
			ax.fill_between([min_x_value, max_x_value], [y_max, y_max], color='red', alpha=0.3)     
		
	# Add overall value
	#val = _compute_parameter(nn, parameter_func)
	#ax.hlines(val, 0, result[-1], linestyles='--', linewidth=0.7)
	...
	if mode == 'normal':
		...
		return result, parameter_values, parameter
		...
		if mode == 'normal':
			...
			args = (fig, )
			names = ("time_varying_%s" % parameter, )
			return biosppy.utils.ReturnTuple(args, names), result, parameter_values, parameter
